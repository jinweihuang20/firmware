...
ProcessMessage AutoReplyModule::handleReceived(const meshtastic_MeshPacket &mp)
{
    ...
    else if (incoming.find("@pubdm") != std::string::npos)
    {
        LOG_DEBUG("@pubdm command->Telemetry info send to mesh.");
        deviceTelemetryModule.sendTelemetry(NODENUM_BROADCAST, false);
        replyMessage += "Telemetry info send to mesh.";
    }
    else
    {
        return ProcessMessage::CONTINUE;
